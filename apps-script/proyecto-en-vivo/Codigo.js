...
const GONSOL_SUPPLIERS = [
  ...
  },
  {
    ...
    invoiceNumber: function(text, upper) {
      // Ej: 26F00768.
      var match = upper.match(/\b(\d{2}\s*F\s*\d{4,6})\b/);
...
function extractPurchaseInvoiceData_(text, originalFileName) {
  ...
  // Ademas exigimos que el total sea coherente (evita registrar lecturas erroneas).
  const totalOk = isTotalSane_(total, tax.base, tax.ivaAmount);
  const requiresReview = supplierKnown ? (!date || !total || !totalOk) : (missing.length > 0 || !totalOk);
  ...
  if (total && !totalOk) {
    observations = (observations ? observations + ' · ' : '')
      + 'Total dudoso (leido ' + total + ', base ' + (tax.base || '?') + '): revisar importe.';
  }
...
function extractTotal_(text) {
  ...
  const heuristico = maxMoneyMatch_(text, patterns);
  // Mejora: si base e IVA se leen bien, el total real es base+IVA. Buscamos ese
  // valor exacto entre los importes del documento (evita el bug del "0,56").
  const base = extractBase_(text);
  const iva = extractIvaAmount_(text);
  if (base && iva) {
    const objetivo = roundMoney_(Number(base) + Number(iva));
    const re = /(\d{1,6}(?:[.,]\d{3})*[.,]\d{2})/g;
    let m;
    while ((m = re.exec(text)) !== null) {
      const v = parseMoney_(m[1]);
      if (v !== '' && Math.abs(Number(v) - objetivo) <= 0.05) return roundMoney_(Number(v));
    }
  }
  return heuristico;
}

// Evita registrar totales imposibles (bug "0,56"): el total no puede ser menor
// que la base, y si hay base e IVA debe cuadrar con base+IVA.
function isTotalSane_(total, base, ivaAmount) {
  total = Number(total) || 0;
  if (total <= 0) return false;
  base = Number(base) || 0;
  ivaAmount = Number(ivaAmount) || 0;
  if (base && total < base - 0.01) return false;
  if (base && ivaAmount && Math.abs(total - (base + ivaAmount)) > 0.02) return false;
  return true;
}
