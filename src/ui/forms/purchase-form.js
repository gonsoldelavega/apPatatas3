(function(global){
  function openPurchaseForm(ctx, id){
    ...
    global.AppUIModal.openModal(
      ...
      "Registra la compra manualmente o escanea la factura con IA para rellenar automÃ¡ticamente",
      `<form id="purchaseForm" class="sheet-grid">
        ...
        <div class="field"><label>Fecha emisión</label><input name="date" type="date" value="${ctx.esc(draft.date)}"></div>
        ...
        <div class="field"><label>Producto (opcional)</label><select name="productId"><option value="">Sin afectar stock</option>${ctx.state.products.map(p => `<option value="${p.id}" ${draft.productId === p.id ? "selected" : ""}>${ctx.esc(p.name)}</option>`).join("")}</select></div>
        <div class="field"><label>Cantidad (si afecta stock)</label><input name="quantityInput" type="number" step="0.01" min="0" value="${ctx.esc(draft.quantityInput)}" placeholder="Ej. 25"></div>
        ...
        <div class="field" style="grid-column:1/-1;">
          ...
          <div class="hint">Adjunta aquí el PDF o la imagen exportada desde Adobe Scan.</div>
        ...
        body.querySelector("#scanInvoiceInput").addEventListener("change", async e => {
          ...
          try {
            ...
            const response = await fetch("https://api.anthropic.com/v1/messages", {
              method: "POST",
              headers: { "Content-Type": "application/json", "x-api-key": ctx.getAnthropicKey(), "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" },
              body: JSON.stringify({
...
Si el proveedor o producto de la factura coincide aproximadamente con alguno de la lista, usa el nombre exacto de la lista.`
            ...
            if (extracted.supplierName) {
              const match = ctx.state.suppliers.find(s => s.name.toLowerCase().includes(extracted.supplierName.toLowerCase()) || extracted.supplierName.toLowerCase().includes(s.name.toLowerCase()));
              if (match) form.elements.supplierId.value = match.id;
            }
            if (extracted.productName) {
              const match = ctx.state.products.find(p => p.name.toLowerCase().includes(extracted.productName.toLowerCase()) || extracted.productName.toLowerCase().includes(p.name.toLowerCase()));
              if (match) {
                form.elements.productId.value = match.id;
                if (extracted.quantity != null) form.elements.quantityInput.value = extracted.quantity;
              }
            }
            syncSummary();
            statusEl.textContent = "âœ… Factura analizada. Revisa los datos.";
          } catch (err) {
            body.querySelector("#scanInvoiceStatus").textContent = "âŒ No se pudo analizar la factura. Rellena manualmente.";
        ...
        actions.querySelectorAll("button").forEach(btn => btn.addEventListener("click", () => {
          ...
          const affectsStock = !!data.productId && ctx.n(data.quantityInput) > 0;
          const quantity = affectsStock ? Math.max(ctx.n(data.quantityInput), 0) : 1;
  ...
  function renderAttachment(attachment, ctx){
    if(!attachment){
      return '<div class="hint">Aún no has añadido ningún documento.</div>';
    }
