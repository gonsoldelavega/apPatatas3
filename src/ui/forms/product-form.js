(function(global){
  function openProductForm(ctx, id){
    const product = id ? ctx.getProduct(id) : {
      id:ctx.uid("pro"),
      name:"",
      category:"",
      price:0,
    ...
    global.AppUIModal.openModal(product.name ? "Editar producto" : "Nuevo producto", "El stock real puede compartirse por familias como patata agria", `<form id="productForm" class="sheet-grid">
      ...
      <div class="field"><label>Precio</label><input name="price" type="number" step="0.01" min="0" value="${ctx.esc(product.price)}" required></div>
      <div class="field"><label>IVA</label><input name="iva" type="number" step="0.01" min="0" max="100" value="${ctx.esc(product.iva)}"></div>
      ...
      actions.querySelectorAll("button").forEach(btn => btn.addEventListener("click", async () => {
        ...
        try{
          await ctx.saveEntity("products", {
            ...product,
            ...data,
            stockGroup:data.stockGroup.trim(),
            price:ctx.n(data.price),
            iva:ctx.n(data.iva),
            ivaPct:ctx.n(data.iva),
            stockBase:ctx.n(data.stockBase),
            stockMin:ctx.n(data.stockMin)
          }, id);
          global.AppUIModal.closeModal();
          ctx.toast("Producto guardado");
        }catch(error){
          console.error("No se pudo guardar el producto", error);
          ctx.toast("No se pudo guardar el producto. Revisa la conexion.");
        }finally{
          btn.disabled = false;
