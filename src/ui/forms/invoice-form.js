(function(global){
  function openInvoiceForm(ctx, id, preset = null){
    ...
    global.AppUIModal.openModal(id ? "Editar factura" : "Nueva factura", "Un \u00fanico bloque l\u00f3gico de per\u00edodo y plantilla editable", `<form id="invoiceForm" class="sheet-grid">
      ...
      <div class="field"><label>N\u00famero</label><input name="number" value="${ctx.esc(invoice.number)}" placeholder="${ctx.esc(ctx.composeInvoiceNumber(ctx.state.settings.nextInvoiceNumber))}" required></div>
      <div class="field"><label>Fecha de emisi\u00f3n interna</label><input name="issueDate" type="date" value="${ctx.esc(invoice.issueDate)}"></div>
      ...
      <div class="field"><label>Periodo de facturaci\u00f3n \u00b7 inicio</label><input name="periodStart" type="date" value="${ctx.esc(invoice.periodStart)}"></div>
      <div class="field"><label>Periodo de facturaci\u00f3n \u00b7 fin</label><input name="periodEnd" type="date" value="${ctx.esc(invoice.periodEnd)}"></div>
      ...
      <div class="field" style="grid-column:1/-1;"><label>L\u00edneas de factura</label><div id="invoiceLines" class="line-list"></div></div>
      ...
      actions.querySelectorAll("button").forEach(btn => btn.addEventListener("click", () => {
        ...
        if(!seq) return ctx.toast("N\u00famero de factura inv\u00e1lido. Usa algo como FAC-080/2026");
        ctx.bumpNextInvoiceNumber(seq);
        ctx.saveEntity("invoices", { ...invoice, ...data, amountPaid:ctx.n(data.amountPaid), showPaymentTerms:data.showPaymentTerms === "true", lines }, id);
