...
import type { ContactCreate, ContactListQuery, ContactPatch, ContactType } from "./types.js";

const fields = ["type", "legalName", "tradeName", "taxId", "email", "phone", "address", "notes", "isActive"] as const;

function contactType(value: unknown): ContactType {
  if (value !== "customer" && value !== "supplier" && value !== "both") throw new HttpError("invalid_request", 400);
  return value;
}

export function validateContactCreate(body: Record<string, unknown>): ContactCreate {
...
export function validateContactPatch(body: Record<string, unknown>): ContactPatch {
  ...
  if (body.legalName !== undefined) result.legalName = requiredText(body.legalName, 200);
...
export function validateContactList(url: URL): ContactListQuery {
  const query: ContactListQuery = listQuery(url, ["name", "createdAt", "updatedAt"]);
