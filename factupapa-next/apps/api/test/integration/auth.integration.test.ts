...
async function jsonRequest(
  ...
  const response = await fetch(`${baseUrl}${path}`, {
    ...
    headers: {
      ...
      ...(refreshToken ? { cookie: `factupapa_refresh=${refreshToken}` } : {}),
    },
    ...(body === undefined
      ? {}
      : { body: JSON.stringify(refreshToken ? {} : body) }),
  });
  if (!path.startsWith("/auth/") || !response.ok || response.status === 204)
    return response;
  ...
  return new Response(
    JSON.stringify({
      ...payload,
      ...(cookie ? { refreshToken: decodeURIComponent(cookie) } : {}),
    }),
...
before(async () => {
  assert.ok(
    databaseUrl,
    "DATABASE_URL del rol API es obligatoria para integración",
  );
  assert.ok(
    databaseAdminUrl,
    "DATABASE_ADMIN_URL es obligatoria para preparar la integración",
  );
  ...
  const secondPasswordHash = await hashPassword(
    "second-integration-password-2026",
  );
  const second = await adminDatabase.pool.query<{
    company_id: string;
    user_id: string;
  }>(
    `with company as (
  ...
  const memberships = await adminDatabase.pool.query<{
    company_id: string;
    user_id: string;
    email: string;
  }>(
  ...
  const firstMembership = memberships.rows.find(
    (row) => row.email === "owner@example.test",
  );
  const secondMembership =
    memberships.rows.find((row) => row.email === "owner-b@example.test") ??
    second.rows[0];
  ...
  secondTenant = {
    companyId: secondMembership.company_id,
    userId: secondMembership.user_id,
  };
...
test("RLS aísla estrictamente dos empresas y el contexto transaccional", async (context) => {
  await context.test(
    "todas las tablas empresariales tienen RLS forzado y políticas",
    async () => {
  ...
  await context.test(
    "el rol API no es propietario, superusuario ni BYPASSRLS",
    async () => {
      ...
      const migrator = await adminDatabase.pool.query<{
        rolcanlogin: boolean;
        rolbypassrls: boolean;
      }>(
        "select rolcanlogin, rolbypassrls from pg_roles where rolname = 'factupapa_migrator'",
      );
      assert.deepEqual(migrator.rows[0], {
        rolcanlogin: false,
        rolbypassrls: true,
      });
      await assert.rejects(
        apiDatabase.pool.query(
  ...
  await context.test("cada usuario solo lee su empresa", async () => {
    ...
    assert.equal(
      own.rows.some((row) => row.id === secondContactId),
      false,
    );
  });

  await context.test(
    "WITH CHECK bloquea insert y cambio de company_id",
    async () => {
      await assert.rejects(
        withTenantTransaction(apiDatabase.pool, firstTenant, (client) =>
          client.query(
            "insert into contacts(company_id, kind, legal_name) values ($1, 'customer', 'Cross tenant')",
      ...
      await assert.rejects(
        withTenantTransaction(apiDatabase.pool, firstTenant, (client) =>
          client.query("update contacts set company_id = $1 where id = $2", [
            secondTenant.companyId,
            firstContactId,
          ]),
        ),
  ...
  await context.test("DELETE no puede afectar filas ajenas", async () => {
    ...
    const stillExists = await adminDatabase.pool.query(
  ...
  await context.test(
    "el pool no conserva contexto después de commit",
    async () => {
      ...
      try {
        ...
        assert.equal(
          (await client.query("select id from contacts")).rowCount,
          1,
        );
  ...
  await context.test(
    "rollback elimina el contexto antes de reutilizar conexión",
    async () => {
      const client = await apiDatabase.pool.connect();
      try {
        await client.query("begin");
        ...
        await assert.rejects(
          client.query("select definitely_missing_column from contacts"),
          /definitely_missing_column/i,
        );
        ...
        const secondOwn = await client.query<{ id: string }>(
          "select id from contacts",
        );
        assert.deepEqual(
          secondOwn.rows.map((row) => row.id),
          [secondContactId],
        );
        await client.query("rollback");
      } finally {
  ...
  await context.test(
    "memberships y auditoría están aisladas por empresa y usuario",
    async () => {
      const firstRows = await withTenantTransaction(
        apiDatabase.pool,
        firstTenant,
        async (client) => ({
          memberships: await client.query("select * from memberships"),
          audits: await client.query(
            "select * from audit_events where company_id is not null",
          ),
        }),
      );
      assert.equal(firstRows.memberships.rowCount, 1);
      assert.ok(
        firstRows.audits.rows.every(
          (row) => row.company_id === firstTenant.companyId,
        ),
      );
    },
  );
...
test("autenticación conserva login, rotación, reutilización, logout y /me", async (context) => {
  ...
  await context.test(
    "el bootstrap no puede ejecutarse una segunda vez",
    async () => {
      await assert.rejects(
  ...
  await context.test(
    "credenciales inválidas no revelan si el email existe",
    async () => {
      const unknown = await jsonRequest("/auth/login", {
        email: "unknown@example.test",
        password: "invalid-password-value",
      });
      const incorrect = await jsonRequest("/auth/login", {
        email: "owner@example.test",
        password: "invalid-password-value",
      });
  ...
  await context.test(
    "el segundo usuario inicia sesión en su propia empresa",
    async () => {
  ...
  await context.test(
    "auth_sessions solo expone sesiones de la empresa y usuario actuales",
    async () => {
      const firstSessions = await withTenantTransaction(
        apiDatabase.pool,
        firstTenant,
        (client) =>
          client.query<{ company_id: string; user_id: string }>(
            "select company_id, user_id from auth_sessions",
          ),
      );
      const secondSessions = await withTenantTransaction(
        apiDatabase.pool,
        secondTenant,
        (client) =>
          client.query<{ company_id: string; user_id: string }>(
      ...
      assert.ok(
        firstSessions.rows.every(
          (row) =>
            row.company_id === firstTenant.companyId &&
            row.user_id === firstTenant.userId,
        ),
      );
      assert.ok(
        secondSessions.rows.every(
          (row) =>
            row.company_id === secondTenant.companyId &&
            row.user_id === secondTenant.userId,
        ),
      );
    },
  );
  ...
  await context.test(
    "refresh rota y PostgreSQL solo conserva hashes",
    async () => {
      ...
      const stored = await adminDatabase.pool.query<{
        refresh_token_hash: string;
      }>("select refresh_token_hash from auth_sessions");
      assert.ok(
        stored.rows.every((row) =>
          /^[a-f0-9]{64}$/.test(row.refresh_token_hash),
        ),
      );
  ...
  await context.test(
    "reutilizar un refresh revocado invalida toda la familia",
    async () => {
      assert.equal(
        (
          await jsonRequest("/auth/refresh", {
            refreshToken: first.refreshToken,
          })
        ).status,
        401,
      ...
      assert.equal(
        (await jsonRequest("/me", undefined, rotated.accessToken)).status,
        401,
      );
    },
  );
  ...
  await context.test(
    "logout revoca la sesión y bloquea /me y refresh",
    async () => {
      const tokens = await login();
      assert.equal(
        (
      ...
      assert.equal(
        (await jsonRequest("/me", undefined, tokens.accessToken)).status,
        401,
      );
      assert.equal(
        (
  ...
  await context.test(
    "auditoría no contiene tokens ni contraseñas",
    async () => {
      const result = await adminDatabase.pool.query<{
        action: string;
        after_data: unknown;
      ...
      for (const action of [
        ...
        assert.ok(
          ...
          `Falta auditoría ${action}`,
        );
      }
      assert.equal(JSON.stringify(result.rows).includes("fp_rt_"), false);
      assert.equal(
        JSON.stringify(result.rows).includes("integration-password"),
        false,
      );
    },
  );
