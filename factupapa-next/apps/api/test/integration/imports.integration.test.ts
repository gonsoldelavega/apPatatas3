...
const jwtSecret =
  process.env.JWT_SECRET ?? "imports-integration-jwt-secret-at-least-32-bytes";
...
async function validate(
  ...
  const response = await request(
    "POST",
    "/imports/validate",
    { entityType, sourceFormat, content },
    token,
  );
  if (response.status !== 201)
    throw new Error(
      `validate_failed:${response.status}:${await response.text()}`,
    );
...
async function expectStatus(
  ...
  if (response.status !== expected)
    throw new Error(
      `unexpected_status:${response.status}:expected_${expected}:${await response.text()}`,
    );
}
...
before(async () => {
  ...
  await bootstrapInitialAccount(adminDatabase.pool, {
    companyName: "Import Company A",
    email: "imports-a@example.test",
    displayName: "Import Owner A",
    password: "imports-password-a",
  });
  ...
  const imports = new ImportService(apiDatabase.pool, {
    maximumBytes: 32_768,
    maximumRows: 100,
    previewRows: 10,
  });
...
test("importación segura tenant de contactos, productos y precios", async (context) => {
  ...
  await context.test(
    "rutas autenticadas, CSV válido y rechazo de company_id",
    async () => {
      ...
      assert.equal(
        ((contactBatch.rows as Entity[])[0]?.normalizedData as Entity).email,
  ...
  await context.test(
    "confirma una vez, reusa checksum y bloquea confirmación repetida",
    async () => {
      const confirm = await request(
        "POST",
        `/imports/${contactBatch.id}/confirm`,
  ...
  await context.test(
    "detecta DB y aplica skip_existing, fail_on_conflict y update_existing",
    async () => {
      const skipBatch = await validate(
        "contacts",
        "json",
        JSON.stringify([
          {
            type: "both",
            legalName: "No debe sobrescribir",
            taxId: "IMP-A-001",
      ...
      const updateBatch = await validate(
        "contacts",
        "json",
        JSON.stringify([
          {
      ...
      assert.equal(
        (
          await request(
            "POST",
            `/imports/${updateBatch.id}/confirm`,
      ...
      const contacts = (await (
        await request("GET", "/contacts?search=IMP-A-001", undefined, tokenA)
  ...
  await context.test(
    "JSON de productos conserva precisión y doble confirmación concurrente se bloquea",
    async () => {
      productBatch = await validate(
        "products",
        "json",
        JSON.stringify([
          {
      ...
      const results = await Promise.all([
        request(
          "POST",
          `/imports/${productBatch.id}/confirm`,
          { strategy: "fail_on_conflict" },
          tokenA,
        ),
        request(
      ...
      const products = (await (
        await request("GET", "/products?search=IMP-SKU-001", undefined, tokenA)
      ).json()) as { items: Entity[] };
  ...
  await context.test(
    "precios resuelven NIF y SKU, rechazan inexistentes y aplican valor exacto",
    async () => {
      ...
      const prices = (await (
        await request(
          "GET",
          `/contacts/${contacts.items[0]?.id}/products?search=IMP-SKU-001`,
          undefined,
          tokenA,
        )
      ).json()) as { items: Entity[] };
  ...
  await context.test(
    "cancelación, listado y aislamiento entre empresas",
    async () => {
      const cancelled = await validate(
        "products",
        "csv",
        "name,sku,unit,sale_price,tax_rate\nCancelado,CANCEL-1,unit,1.0000,21\n",
      );
      assert.equal(
        (await request("POST", `/imports/${cancelled.id}/cancel`, {}, tokenA))
          .status,
        204,
      );
      assert.equal(
      ...
      assert.equal(
        (await request("GET", `/imports/${contactBatch.id}`, undefined, tokenB))
          .status,
        404,
      );
      const listB = (await (
        await request("GET", "/imports", undefined, tokenB)
  ...
  await context.test(
    "un fallo inesperado revierte datos y conserva diagnóstico",
    async () => {
      const batch = await validate(
        "products",
        "json",
        JSON.stringify([
          {
            name: "Producto que hará rollback",
            sku: "ROLLBACK-1",
            unit: "unit",
            salePrice: "1.0000",
            taxRate: "21",
      ...
      const products = (await (
        await request("GET", "/products?search=ROLLBACK", undefined, tokenA)
      ).json()) as { total: number };
      assert.equal(products.total, 0);
      const detail = (await (
        await request("GET", `/imports/${batch.id}`, undefined, tokenA)
      ).json()) as Entity;
  ...
  await context.test(
    "audita validar, confirmar, cancelar y fallar sin contenido completo",
    async () => {
      const events = await adminDatabase.pool.query<{
        action: string;
      ...
      for (const action of [
        ...
        assert.ok(
          events.rows.some((row) => row.action === action),
          `Falta ${action}`,
        );
      }
      assert.ok(
        events.rows.every((row) => !("content" in (row.afterData ?? {}))),
      );
      assert.equal(
        (await apiDatabase.pool.query("select id from import_batches"))
          .rowCount,
        0,
      );
      assert.equal(
        (await apiDatabase.pool.query("select id from import_batch_rows"))
          .rowCount,
        0,
      );
      assert.equal(
