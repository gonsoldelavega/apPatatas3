...
test("IRPF a compensar o régimen no aplicable no genera pago",()=>{assert.equal(irpf130Forecast({income:1000,expenses:2000,rate:20,previousPayments:0,withholdings:0,minoration:0,applicable:true}).result,0);assert.equal(irpf130Forecast({income:1000,expenses:0,rate:20,previousPayments:0,withholdings:0,minoration:0,applicable:false}).result,0)});
