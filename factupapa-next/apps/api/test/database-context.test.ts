...
function fakeDatabase(failOperation = false): { pool: Pool; statements: string[]; released: () => boolean } {
  const statements: string[] = [];
  let wasReleased = false;
  const client = {
    async query(sql: string) {
      statements.push(sql.replace(/\s+/g, " ").trim());
      if (failOperation && sql === "operation") throw new Error("operation failed");
...
test("el contexto se establece localmente dentro de begin/commit", async () => {
  ...
  await withTenantTransaction(fake.pool, context, (client) => client.query("operation"));
  assert.equal(fake.statements[0], "begin");
  assert.match(fake.statements[1]!, /set_config\('app\.current_company_id'.*true\)/);
  assert.match(fake.statements[1]!, /set_config\('app\.current_user_id'.*true\)/);
...
test("una operación fallida hace rollback y libera la conexión", async () => {
  ...
  await assert.rejects(
    withTenantTransaction(fake.pool, context, (client) => client.query("operation")),
    /operation failed/,
  );
