...
export function ProductFormPage() {
  ...
  useEffect(() => {
    if (product.data)
      reset({
        ...
        salePrice: formatQuantity(product.data.salePrice),
  ...
  useEffect(() => {
    if (!id && preferences.data && !dirtyFields.taxRate)
      setValue("taxRate", formatQuantity(preferences.data.defaultTaxRate));
  }, [dirtyFields.taxRate, id, preferences.data, setValue]);
  const save = useMutation({
    ...
    onSuccess: async () => {
      ...
      navigate("/catalogo/productos", { replace: true });
    },
  });
  ...
  return (
    <div className="page form-page">
      <header className="form-page__header">
        ...
        <div>
          <p className="eyebrow">{id ? "Editar" : "Alta"}</p>
      ...
      <form onSubmit={handleSubmit((values) => save.mutate(values))} noValidate>
        ...
        <section className="form-card">
          ...
          {!id && watch("salePrice") && (
            <p className="hint">
              Los importes se envían como texto decimal para conservar la
              precisión.
            </p>
          )}
        </section>
        {save.isError && (
          <div className="form-alert" role="alert">
            No se ha podido guardar. Revisa los datos o el SKU duplicado.
          </div>
        )}
        {id && product.data?.isActive && (
          <Button
            type="button"
            variant="danger"
            icon={<Trash2 />}
            ...
            onClick={() => {
              if (
                window.confirm(
                  "¿Dar de baja este producto? Seguirá en el historial.",
                )
              )
                deactivate.mutate();
            }}
