  ...
  return (
    <section className="catalog-section" aria-labelledby="contacts-title">
      <div className="section-heading">
        ...
        <Link className="compact-action" to="/contactos/nuevo">
          <Plus />
          Añadir
        </Link>
      </div>
      ...
      {contacts.isLoading && (
        <div className="card-list" aria-label="Cargando contactos">
          {[1, 2, 3].map((item) => (
      ...
      {contacts.isError && (
        <div className="inline-error" role="alert">
          No se han podido cargar los contactos.{" "}
          <button onClick={() => void contacts.refetch()}>Reintentar</button>
        </div>
      )}
      {contacts.data?.items.length === 0 && (
        <EmptyState
          title="No hay contactos aquí"
          description={
            search
              ? "Prueba con otra búsqueda o cambia el filtro."
              : "Crea tu primer cliente o proveedor para empezar."
          }
          action={
            <Link className="button button--primary" to="/contactos/nuevo">
        ...
        {contacts.data?.items.map((contact) => (
          ...
          >
            <span className="entity-card__icon">
              {contact.type === "supplier" ? <Store /> : <Building2 />}
            </span>
            <span className="entity-card__body">
              <strong>{contact.tradeName || contact.legalName}</strong>
              <small>
                {contact.taxId ||
                  contact.email ||
                  contact.phone ||
              ...
              <span
                className={`status ${contact.isActive ? "status--active" : "status--inactive"}`}
              >
                {contact.isActive ? filterLabels[contact.type] : "Inactivo"}
              </span>
            </span>
      ...
      {contacts.data && contacts.data.total > contacts.data.pageSize && (
        <div className="pagination">
          ...
          <Button
            variant="secondary"
            disabled={page * contacts.data.pageSize >= contacts.data.total}
            onClick={() => setPage((value) => value + 1)}
          >
            Siguiente
          </Button>
