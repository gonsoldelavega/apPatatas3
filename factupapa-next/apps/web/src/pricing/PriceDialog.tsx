...
export function PriceDialog({
  ...
  const {
    ...
    defaultValues: {
      price: formatQuantity(product.specificPrice ?? product.salePrice),
  ...
  const save = useMutation({
    mutationFn: (values: PriceValues) =>
      pricingApi.upsert(contactId, product.id, {
        ...values,
        price: values.price.replace(",", "."),
      }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["prices", contactId] });
      toast.show("Precio específico guardado");
      onClose();
    },
  });
  const remove = useMutation({
    mutationFn: () => pricingApi.deactivate(contactId, product.id),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["prices", contactId] });
      toast.show("Se aplicará el precio general");
      onClose();
    },
  });
  useEffect(() => {
  ...
  return (
    <div className="sheet-backdrop">
      <section
        className="action-sheet price-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          <div>
            <p className="eyebrow">Precio específico</p>
            <h2 id="price-title">{product.name}</h2>
          </div>
          <button
            ref={closeButton}
        ...
        <p className="hint">Precio general: {formatMoney(product.salePrice)}</p>
