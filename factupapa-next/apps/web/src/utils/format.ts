...
function fixed(value: string, decimals: number): string | null {
  ...
  let scaled = BigInt(parts.integer) * factor + BigInt(padded.slice(0, decimals) || "0");
...
export function formatUnitPrice(value: string): string {
  ...
  const fraction = parts.fraction.padEnd(2, "0").replace(/0+$/u, "").padEnd(2, "0");
...
export function formatDocumentNumber(series: string, number: number | null): string {
...
export function annualInvoiceSeries(prefix = "FAC", date = todayLocal()): string {
